...
void tp() {
   ...
   while((uint32_t)entry < (info->mbi->mmap_addr + info->mbi->mmap_length)) {
      ...
      debug("entry mem [0x%llx - 0x%llx] type: %d\n", entry->addr, entry->len + entry -> addr, entry -> type);
      entry++;
   }
   ...
   *ptr_in_reserved_mem = 0xaaaaaaaa;                           // write
   ...
   int *ptr_in_behind_mem;
   ptr_in_behind_mem = (int*)0x100000001;
   debug("Reserved mem (at: 0x100000001):  before: 0x%x ", *ptr_in_behind_mem); // read
   *ptr_in_behind_mem = 0xaaaaaaaa;                           // write
   debug("after: 0x%x\n", *ptr_in_behind_mem);                // check
